...
class SheeplaClient {
    ...
    protected function addShipmentToOrder_createOrdersNode( $document , $orderId , $packagesData = null ) {
        
        $order = $document->createElement( 'order' );
        
        $externalOrderId = $document->createElement( 'externalOrderId' , $orderId );
        
        $order->appendChild( $externalOrderId );
        
        if( is_array( $packagesData ) ) {
            
            $packages = $document->createElement( 'packages' );
            $order->appendChild( $packages );       
            ...
            foreach( $packagesData as $packageData ) {
                ...
                if( isset( $packageData[ 'weight' ] ) ) {
                    
                    $package->appendChild( $document->createElement( 'weight' , $packageData[ 'weight' ] ) );
                }
            }
        }
    ...
    public function addShipmentToOrderEx( $orderId , $packagesData = null ) {
        
        $result = array();
        $request = $this->createRequestDom( 'addShipmentToOrderRequest' , true );
        $orders = $request->createElement( 'orders' );
        $request->documentElement->appendChild( $orders );
        
        
        $orders->appendChild( $this->addShipmentToOrder_createOrdersNode( $request , $orderId , $packagesData ) );
        
        
        $xml = $request->saveXML();
        
        
        $response = $this->toSimpleXMLElement($this->send('addShipmentToOrder', $xml));
        if (isset($response->errors) || !isset($response->orders)) {
            throw new Exception('The response XML structure is wrong');
        }
        ...
        foreach( $response->orders->order as $t ) {
            $result[] = $this->mapXML( $t , $keys );
            if ( !$this->isArrayValid( $t , $keys ) ) {
                throw new Exception( 'The response XML structure is wrong pleas update Client' );
            }
    ...
    public function addShipmentsToOrdersEx( array $ordersData ) {
        ...
        foreach( $ordersData as $orderId => $orderData ) {
            ...
            $orders->appendChild( $this->addShipmentToOrder_createOrdersNode( $request , $orderId , $orderData ) );
        }
        ...
        $response = $this->toSimpleXMLElement( $this->send( 'addShipmentToOrder' , $xml ) );
        if( isset( $response->errors ) || !isset( $response->orders ) ) {
            throw new Exception( 'The response XML structure is wrong. ' . print_r( $response->errors , true ) );
        }
     ...
     * @param int $order
     * @param array|null packages data available keys: weight
     * @return Ambigous <multitype:, multitype:multitype:string  , mixed>
     */
    public function addShipmentToOrder( $order , $packagesData = null ) {
        
        if( is_array( $packagesData ) ) {
            
            return $this->addShipmentToOrderEx( $order , $packagesData );
        }
    ...
    public function getShipmentsDetails($edtns) {
        ...
        $root = $dom->documentElement;
        $element = $dom->createElement('edtns');
        foreach ($edtns as $edtn) {
            $element->appendChild($dom->createElement('edtn', $edtn));
        }
        $root->appendChild($element);
        $xml = $dom->saveXML();
                    ...
                    $tmpValue = '';
                     ...
                     foreach($value as $val){
                        ...
                        if(is_object($val)){
                            ...
                            $tmpValue .= json_encode($val->getData()).'|';
